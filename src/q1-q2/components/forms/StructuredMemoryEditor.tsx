...
function parseFieldValue(previousValue: unknown, nextValue: string): unknown {
...
export function validateMemoryRecords(records: EditableMemoryRecord[]): string | null {
  if (records.length === 0) {
    return '至少保留或新增一条 memory。';
  }

  for (const [index, record] of records.entries()) {
    if (!String(record.value ?? '').trim()) {
      return `第 ${index + 1} 条 memory 的 value 不能为空。`;
    }

    if (!String(record.label ?? '').trim()) {
      return `第 ${index + 1} 条 memory 的 label 不能为空。`;
    }

    if (typeof record.evidence === 'string') {
      return `第 ${index + 1} 条 memory 的 evidence 必须是 JSON object/null，不能是普通字符串。`;
    }
  }
    ...
    onChange(
      memories.map((memory, index) =>
        index === memoryIndex
          ? {
              ...memory,
              [field]: parseFieldValue(memory[field], value),
            }
          : memory,
      ),
    );
