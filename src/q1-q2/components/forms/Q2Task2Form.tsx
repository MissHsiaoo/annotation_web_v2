...
const ISSUE_TYPE_OPTIONS = [
  { value: 'pair_error', label: '配对错误' },
  { value: 'update_type_error', label: '更新类型判断错误' },
  { value: 'severity_error', label: '严重度判断错误' },
  { value: 'missing_expected_update', label: '漏判应有更新' },
  { value: 'false_hallucination', label: '误判为幻觉' },
...
interface Q2Task2FormProps {
  ...
  onDraftChange?: (annotation: Q2Task2AnnotationRecord) => void;
...
export function Q2Task2Form({
  ...
  const [record, setRecord] = useState<Q2Task2AnnotationRecord>(startingValue);
  ...
  const handleSave = () => {
    if (mode === 'judge_visible') {
      ...
      ) as Q2Task2AnnotationRecord['annotationsByMode'];
